    ...
    class DBAppointment
    {
        ...
        public static void AddAppointment()
        {
            DBConnection.SqlString = $"INSERT INTO appointment (appointmentId, customerId, consultantId, type, start, end, createDate, createdBy, lastUpdate, lastUpdateBy)" +
                $" VALUES ({AppointmentID + 1}, {AddUpdateAppointments.CustomerID}, {AddUpdateAppointments.ConsultantID}, \"{AddUpdateAppointments.AppointmentType}\", \"{AddUpdateAppointments.StartTime.ToUniversalTime().ToString("yyyy-MM-dd H:mm:ss")}\", \"{AddUpdateAppointments.EndTime.ToUniversalTime().ToString("yyyy-MM-dd H:mm:ss")}\", CURRENT_TIMESTAMP(), \"{Login.UserName}\", CURRENT_TIMESTAMP(), \"{Login.UserName}\")";
            DBConnection.Cmd = new MySqlCommand(DBConnection.SqlString, DBConnection.Conn);
